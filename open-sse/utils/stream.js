...
export function createSSEStream(options = {}) {
  ...
  // State for extracting  thinking... response to reasoning_content across SSE chunks
  let thinkBuf = "";
  let inThink = false;
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
          ...
          if (trimmed.startsWith("data:") && trimmed.slice(5).trim() !== "[DONE]") {
            try {
              ...
              // Extract  thinking... response from content to reasoning_content (MiniMax M3)
              if (typeof delta?.content === "string") {
                let t = delta.content;
                let gotThink = false;
                if (inThink) {
                  let ei = t.indexOf(" response");
                  if (ei >= 0) {
                ...
                if (!inThink) {
                  let si = t.indexOf(" thinking");
                  if (si >= 0) {
                    let aft = t.slice(si + 7), ei = aft.indexOf(" response");
                    if (ei >= 0) {
